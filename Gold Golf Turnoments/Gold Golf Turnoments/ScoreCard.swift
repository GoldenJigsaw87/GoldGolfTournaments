...
struct ScorecardView: View {
    ...
    private func joinCardAsCurrentUser() {
        ...
        // First check if user is already in the card
        if card.players.contains(where: { $0.id == user.id }) {
            print("ℹ️ User \(user.id) is already in card \(roomCode)")
            hasJoinedCard = true
            return
        }
        ...
        Task {
            do {
                ...
                print("👤 Attempting to add current user \(player.username) to card \(roomCode)")
                isLoading = true
                try await firebaseService.addPlayerToCard(roomId: roomCode, player: player)
                await MainActor.run {
                    hasJoinedCard = true
                    isLoading = false
                    print("✅ Current user successfully added to card \(roomCode)")
                }
            } catch {
                await MainActor.run {
                    isLoading = false
                    errorMessage = "Failed to join card: \(error.localizedDescription)"
                    print("❌ Failed to add current user to card: \(error)")
                }
            }
        }
    }
